...
class DataloaderNymeria(data.Dataset):
    def __init__(self,
    ...
    def divide_clip(self, seq_name):
        preprocessed_amass_joints_dir = os.path.join(self.preprocessed_amass_root, 'pose_data_fps_30', 'Nymeria', seq_name)
        preprocessed_amass_smpl_dir = os.path.join(self.preprocessed_amass_root, 'smpl_data_fps_30', 'Nymeria', seq_name)
        
        # Nymeria sequence folder contains npy files.
        seqs_path = sorted(glob.glob(os.path.join(preprocessed_amass_joints_dir, '*.npy')))
        
        for path in seqs_path:
            npy_name = os.path.basename(path)
            path_joints = path
            path_smplx = os.path.join(preprocessed_amass_smpl_dir, npy_name)
            
            if not os.path.exists(path_smplx):
                # Try recursive search if file structure is deeper or different
                # But typically it should mirror
                continue
            ...
            try:
                ...
                print(f"[ERROR] Failed to load {path_joints} or {path_smplx}: {e}")
                continue
    ...
    def read_data(self):
        split_file = os.path.join(self.preprocessed_amass_root, 'nymeria_splits', self.split + '.txt')
        if not os.path.exists(split_file):
             print(f"[ERROR] Split file not found: {split_file}")
             # Fail gracefully or empty list
             sequences = []
        else:
    ...
    def create_body_repr(self):
        ...
        for i in tqdm(range(0, self.n_samples, self.spacing)):
            if i >= len(self.joints_clip_list): break 
        ...
        if self.split == 'train':
            ...
            with open(os.path.join(save_dir, 'Nymeria_mean.pkl'), 'wb') as result_file:
                pkl.dump(self.Mean_dict, result_file, protocol=2)
            with open(os.path.join(save_dir, 'Nymeria_std.pkl'), 'wb') as result_file:
                pkl.dump(self.Std_dict, result_file, protocol=2)

        elif self.split == 'test':
            ######## load mean/std stats from the training data
            with open(os.path.join(save_dir, 'Nymeria_mean.pkl'), 'rb') as f:
                self.Mean_dict = pkl.load(f)
            with open(os.path.join(save_dir, 'Nymeria_std.pkl'), 'rb') as f:
