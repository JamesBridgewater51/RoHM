...
group.add_argument('--saved_data_path', type=str,
                   default='data/test_results_release/results_amass_full/test_amass_full_grad_True_mask_random_joints_max15_noise_3_iter_2_iter2trajnoisy_True_iter2posenoisy_True_earlystop_False_seed_0.pkl',
                   help='path to saved test results')
# [Surgical Inject]: mask_scheme default and choices updated for random_joints_max15
group.add_argument("--mask_scheme", default='random_joints_max15', type=str, choices=['lower', 'full', 'random_joints_max15'], help='occlusion setup for test')
...
group.add_argument("--render_save_path", default='render_imgs/render_amass/mask_random_joints_max15_noise_3', type=str, help='path to save render images')
...
if __name__ == "__main__":
    ...
    if args.mask_scheme == 'lower':
        ...
        print('mpjpe_global_vis / occ (mm): {:0.1f} / {:0.1f}'.format(np.mean(joints_mpjpe_global_vis) * 1000, np.mean(joints_mpjpe_global_invis) * 1000))
    elif args.mask_scheme == 'random_joints_max15':
        # [Surgical Inject]: For random_joints_max15, visible/occluded split is not meaningful
        # because the mask varies per sample at test time and is not saved in the pickle.
        # Report only global MPJPE (already printed above).
        print('[INFO] random_joints_max15: per-joint visible/occluded split not applicable (mask varies per sample).')
        ...
        for bs in range(0, n_seq, 1):
            if bs % args.vis_interval == 0:
                ...
                for t in range(clip_len):
                    ############################################# body skeletons
                    # [Surgical Inject]: random_joints_max15 has no fixed mask_joint_id for visualization
    ...
    if args.render:
        ...
        # [Surgical Inject]: random_joints_max15 has no fixed mask_joint_id for rendering
        cur_mask_joint_id = None
        ...
        for bs in range(0, n_seq, args.render_interval):
            ...
            for t in range(0, clip_len, 1):
                ...
                # [Surgical Inject]: random_joints_max15 uses rec_occ material (no fixed visible/occluded split)
                ...
                ####### render: noisy body
                color_noisy_body = render_img(r, scene_noisy_body, alpha=1.0)
                color_noisy_skel = render_img(r, scene_noisy_skel, alpha=1.0)
