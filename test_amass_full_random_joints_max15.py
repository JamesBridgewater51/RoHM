...
# [Surgical Inject]: mask_scheme is always 'random_joints_max15' for this script
group.add_argument("--mask_scheme", default='random_joints_max15', type=str, help='occlusion scheme for poseNet: random_joints_max15')
...
def main(args):
    ...
    for test_step in tqdm(range(len(test_pose_dataset) // args.batch_size + 1)):
        ...
        for iter_idx in range(args.sample_iter):
            ...
            ######### apply random_joints_max15 occlusion masks
            # [Surgical Inject]: Replace lower/upper/full masking with random_joints_max15
            ...
            if iter_idx < mask_iter_num:
                traj_feat_dim_pose = test_pose_dataset.traj_feat_dim
                # [Surgical Inject]: random_joints_max15 -- randomly mask 1-15 joints per batch
                ...
                for i in range(bs):
                    for k in range(3):
                        test_batch_pose['cond'][i, :, traj_feat_dim_pose + mask_joint_id[i] * 3 + k] = 0.
                    for k in range(3):
                        test_batch_pose['cond'][i, :, traj_feat_dim_pose + 22 * 3 + mask_joint_id[i] * 3 + k] = 0.
                    # [Surgical Inject]: Guard m_id > 0: pelvis (joint 0) has no entry in smplx_body_pose_6d (21 joints, indexed 1-21)
                    for m_id in mask_joint_id[i]:
                        if m_id > 0:
                            for k in range(6):
                                test_batch_pose['cond'][i, :, traj_feat_dim_pose + 22 * 3 + 22 * 3 + (m_id - 1) * 6 + k] = 0.
                    # [Surgical Inject]: Conditionally zero foot contact per sample
                    if 7 in mask_joint_id[i] or 10 in mask_joint_id[i]:  # left foot
                        test_batch_pose['cond'][i, :, -4:-2] = 0.
                    if 8 in mask_joint_id[i] or 11 in mask_joint_id[i]:  # right foot
                        test_batch_pose['cond'][i, :, -2:] = 0.
                if args.input_noise:
        ...
        # [Surgical Inject]: Use mask_scheme='random_joints_max15' in save dir name
