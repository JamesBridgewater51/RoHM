...
from data_loaders.dataloader_nymeria import DataloaderNymeria
...
group.add_argument('--dataset_root', type=str, default='/home/minghao/src/robotflow/RoHM/datasets/Nymeria_smplx_preprocessed', help='path to datas')
...
group.add_argument('--save_root', type=str, default='test_results/results_nymeria_full', help='')
...
def main(args):
    ...
    if args.load_noise:
        # Note: 'data/eval_noise_smplx' path might need to be adjusted if not running from repo root
        # Assuming running from repo root as per typical usage
        ...
        if os.path.exists(noise_pkl_path):
            with open(noise_pkl_path, 'rb') as f:
             ...
             print(f"Warning: Noise file {noise_pkl_path} not found. using None.")
    ...
    test_traj_dataset = DataloaderNymeria(preprocessed_amass_root=args.dataset_root, split='test',
                                        body_model_path=args.body_model_path,
    ...
    for test_step in tqdm(range(len(test_pose_dataset) // args.batch_size + 1)):
        ...
        try:
            ...
            try:
                ...
                break # Should break if dataset is empty or we reached end again?
